...
describe("check the search functionality in google", function() {
    ...
    it("Go to google and search for BMW", async () => {
        driver.get("http://www.google.co.uk"); // navigate to the URL declared
        driver.findElement(By.id("L2AGLb")).click(); //accept the agreement
        await driver.findElement(By.name("q")).sendKeys("BMW", Key.ENTER); //type QA and press enter
        const val = await driver.wait(driver.getTitle(), 1000); // get the title of the webpage and store it into val

        assert.equal(val, "BMW - Google Search"); // to check if the value we got is equal to what we expected!

    });

});

// testing bing functionality - still using chrome driver not sure why this wasnt Edge
describe("Testing Bing", function () {
    ...
    it("should contain a header saying News", async function () {
      ...
      headerElement = await driver.findElement(
        By.xpath("/html/body/div[7]/header/div/div/nav[2]/ul/li[2]/a/span")
        // /html/body/div[7]/header/div/div/nav[2]/ul/li[2]/a/span
        // /html/body/div/div/header/div[3]/div/div/div/div/div
        // /html/body/div[7]/header
